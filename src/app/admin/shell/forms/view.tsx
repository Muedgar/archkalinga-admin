...
export default function ViewShellItemForm() {
  ...
  const { loading, currentShellItem } = useSelector(
  ...
  const { description, unit, shellSubCategory }: IShellItem = currentShellItem
    ...
    <div className="max-w-3xl mx-auto p-6">
      <Card>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-6">
          ...
          />
          <InfoRow
            icon={<CurlyBracesIcon className="w-4 h-4" />}
            label="Unit"
            value={unit}
          />
          <InfoRow
            ...
            label="Sub category"
            value={shellSubCategory?.name}
          />
          <InfoRow
            icon={<AxeIcon className="w-4 h-4" />}
            label="Category"
            value={shellSubCategory?.shellCategory?.name}
          />
          <InfoRow
            icon={<BanIcon className="w-4 h-4" />}
            label="Phase"
            value={shellSubCategory?.shellCategory?.shellPhase?.name}
          />
        </CardContent>
      </Card>
    </div>
