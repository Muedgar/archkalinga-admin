...
import { GetUserItemUsageFormValues, getUserItemUsageSchema } from '../schemas'
...
import { IProject, IShellItem, IUser } from '@/interfaces'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Separator } from '@/components/ui/separator'

export const GetUserItemUsage = () => {
  const dispatch = useDispatch<AppDispatch>()
  const [isCollapse, setIsCollapse] = useState(true)
  const { users } = useSelector((state: RootState) => state.user)
  const { projects } = useSelector((state: RootState) => state.project)
  const { shellItems } = useSelector((state: RootState) => state.shellItem)
  const { loading, userItemUsageInProject } = useSelector(
    (state: RootState) => state.getUserItemUsage
  )

  const form = useForm<GetUserItemUsageFormValues>({
    resolver: zodResolver(getUserItemUsageSchema),
    defaultValues: {
      userId: '',
      itemId: '',
      projectId: '',
  ...
  const onSubmit = async (data: GetUserItemUsageFormValues) => {
    await dispatch(
      getUserItemUsage({
        ...data,
      })
      ...
      <Card className="mt-4">
        <CardHeader>
          <CardTitle className="text-xl flex items-center gap-2">
            <DrillIcon className="w-5 h-5 text-primary" /> User specific item
            usage in a project
          </CardTitle>
        </CardHeader>
        <Separator />
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-6">
          <InfoRow
            ...
            label="Item"
            value={userItemUsageInProject?.item}
          />
          <InfoRow
            icon={<Scale3D className="w-4 h-4" />}
            label="Unit"
            value={userItemUsageInProject?.unit}
          />
          <InfoRow
            icon={<PlusCircleIcon className="w-4 h-4" />}
            label="Project"
            value={userItemUsageInProject?.project}
          />
          {userItemUsageInProject &&
            userItemUsageInProject?.assignments?.length > 0 && (
              <>
                ...
                <div className="mt-4 px-4">
                  ...
                  <div className="space-y-4">
                    {userItemUsageInProject.assignments.map((assignment) => (
                      <div
                        key={assignment.taskId}
                        className="border p-3 rounded-lg bg-muted/50 shadow-sm"
                      >
                        <InfoRow
                          icon={
                        ...
                        <InfoRow
                          ...
                          label="Amount"
                          value={`${assignment.amount} ${assignment.unit}`}
                        />
                      </div>
                    ))}
                  </div>
