...
import {
  getProjects,
  getShellItems,
  itemAndQuantityBreakdownByTask,
} from '@/store/thunks'
import {
  ArrowRight,
  ChevronDown,
...
export const GetItemTaskBreakdownByTask = () => {
  const dispatch = useDispatch<AppDispatch>()
  const [isCollapse, setIsCollapse] = useState(true)
  const { projects } = useSelector((state: RootState) => state.project)
  const { shellItems } = useSelector((state: RootState) => state.shellItem)
  const { loading, itemTaskBreakdown } = useSelector(
    (state: RootState) => state.itemAndQuantityBreakdownByTask
  )
      ...
      <Card className="mt-4">
        <CardHeader>
          <CardTitle className="text-xl flex items-center gap-2">
            <DrillIcon className="w-5 h-5 text-primary" /> Item and quantity
            breakdown by task
          </CardTitle>
        </CardHeader>
        <Separator />
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-6">
          <InfoRow
            ...
            label="Item"
            value={itemTaskBreakdown?.item}
          />
          <InfoRow
            icon={<Scale3D className="w-4 h-4" />}
            label="Unit"
            value={itemTaskBreakdown?.unit}
          />
          <InfoRow
            icon={<PlusCircleIcon className="w-4 h-4" />}
            label="Project"
            value={itemTaskBreakdown?.project}
          />
          {itemTaskBreakdown?.breakdown &&
            itemTaskBreakdown.breakdown.length > 0 && (
              <>
                <Separator className="my-4" />
                <div className="mt-4 px-4">
                  <h3 className="text-lg font-semibold mb-3">Task Breakdown</h3>
                  <div className="space-y-4">
                    {itemTaskBreakdown.breakdown.map((task) => (
                      <Card key={task.taskId} className="border shadow-sm">
                        <CardContent className="pt-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <InfoRow
                              label="Task Name"
                              value={task.taskName}
                              icon={<DrillIcon className="w-4 h-4" />}
                            />
                            <InfoRow
                              label="Amount Used"
                              value={`${task.amount} ${task.unit}`}
                              icon={<Scale3D className="w-4 h-4" />}
                            />
                          </div>
                        </CardContent>
